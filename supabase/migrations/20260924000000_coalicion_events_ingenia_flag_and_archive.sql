-- KPI: registra si Fundación Ingenia participó en cada evento de Coalición,
-- y agrega la capacidad de archivar (eliminar) un evento — necesaria para
-- poder "mover" un evento hacia otra organización desde el calendario
-- compartido de Networking.

alter table public.coalicion_events
  add column if not exists participo_fundacion_ingenia boolean not null default false;

grant select (participo_fundacion_ingenia) on public.coalicion_events to anon, authenticated;
...
begin
  if p_entity = 'event' then
    ...
    if p_id is null then
      insert into public.coalicion_events (title, event_date, start_time, location, maps_url, status, notes, participo_fundacion_ingenia)
      values (
...
create or replace function public.coalicion_archive_event_public(
  p_id uuid
)
returns boolean
language plpgsql
...
as $$
begin
  update public.coalicion_events set archived_at = now(), updated_at = now()
  where id = p_id and archived_at is null;
  return found;
end;
$$;
...
revoke all on function public.coalicion_archive_event_public(uuid) from public, anon, authenticated;
grant execute on function public.coalicion_save_record_public(text, jsonb, uuid) to service_role;
grant execute on function public.coalicion_archive_event_public(uuid) to service_role;
